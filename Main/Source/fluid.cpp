  ...
  for(ushort c = 0; c < Amount; ++c)
    {
      ...
      GetPicture()->PutPixel(Cords.X, Cords.Y, Color);
      GetPicture()->SetAlpha(Cords.X, Cords.Y, 150 + RAND() % 106);
	...
	if(RAND() % Lumpiness)
	  {
	    ...
	    GetPicture()->PutPixel(Cords.X + game::GetMoveVector(d).X, Cords.Y + game::GetMoveVector(d).Y,
				       MAKE_RGB(GET_RED(Color) + Change[0], GET_GREEN(Color) + Change[1], GET_BLUE(Color) + Change[2]));

	    GetPicture()->SetAlpha(Cords.X + game::GetMoveVector(d).X, Cords.Y + game::GetMoveVector(d).Y, 50 + RAND() % 206);
	  }
    }
}
